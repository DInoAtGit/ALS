...
ratingsMatrixTrain <- question_data_sub_2 %>% select(user = masked_user_id, item = question_id, ratings = points_earned, 9:15,17)
ratingsMatrixTrain$user <- as.factor(ratingsMatrixTrain$user)
ratingsMatrixTrain$item <- as.factor(ratingsMatrixTrain$item)
...
if (!(LoggedInUser %in% question_data[question_data$masked_user_id == LoggedInUser,]$masked_user_id)) {
  ...
  head(CFrecommended,10);strength_tags
}
...
stream_view_data$view_status[stream_view_data$ss == 0] = 1  ##Fully Viewed
stream_view_data$view_status[stream_view_data$ss > 0 & stream_view_data$vs > 0] = -1  ##Partially Viewed
stream_view_data$view_status[stream_view_data$ss > 0 & stream_view_data$vs == 0] = 0 ##Not viewed at all
...
streams_tags = merge(stream_recommend, stream_master, by.x = "deck_id", by.y = "deck_id", all.x = T ) %>% 
  select(deck_id = deck_id, rec_sclae = rec_sclae, tag1 = tag1, tag2=tag2,tag3=tag3,tag4=tag4,tag5=tag5,tag6=tag6,tag7=tag7 ) %>% arrange(rec_sclae)
